...
static void free_list_memory(void)
{
	...
	if (list_empty(&head_list.list))
		no_printk("*****list is empty!!\n");
	else
		no_printk("*****list is NOT empty!!\n");
...
static int fbconfig_open(struct inode *inode, struct file *file)
{
	...
	if (pm_params == NULL) {
		no_printk("fbconfig_open=>pm_params is empty!!\n");
		return -EFAULT;
	}
	...
	unsigned long arg)
{
	...
	case SET_DSI_ID:
	{
		if (arg > PM_DSI_DUAL)
			return -EINVAL;
		pm->dsi_id = arg;
		no_printk("fbconfig=>SET_DSI_ID:%d\n", dsi_id);
	...
	case LCM_TEST_DSI_CLK:
	{
		...
		no_printk("fbconfig=>LCM_TEST_DSI_CLK:%d\n", ret);
...
/* LCM_DRIVER*pLcm_drv=pm->pLcm_drv; */
		...
		if (pLcm_drv != NULL)
			...
			no_printk("fbconfig=>LCM_GET_ID:%x\n", lcm_id);
#endif
	...
	case DRIVER_IC_CONFIG:
	{
		...
		if (copy_from_user(&record_tmp_list->record,
			...
			no_printk("list_add: copy_from_user failed! line:%d\n",
				__LINE__);
	...
	case MIPI_SET_CC:
	{
		...
		if (get_user(enable, (uint32_t __user *) argp)) {
			no_printk("[MIPI_SET_CC]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
	...
	case LCM_GET_DSI_CONTINU:
	{
		...
		no_printk("LCM_GET_DSI_CONTINU=>DSI: %d\n", ret);
	...
	case MIPI_SET_CLK:
	{
		...
		if (get_user(clk, (uint32_t __user *) argp)) {
			no_printk("[MIPI_SET_CLK]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}

		no_printk("LCM_GET_DSI_CLK=>dsi:%d\n", clk);
	...
	case MIPI_SET_SSC:
	{
		...
		if (copy_from_user(&dsi_ssc, (void __user *)argp,
			sizeof(dsi_ssc))) {
			no_printk("[MIPI_SET_SSC]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}

		no_printk("Pmaster:set mipi ssc line:%d\n", __LINE__);
	...
	case LCM_GET_DSI_LANE_NUM:
	{
		...
		no_printk("Panel Master=>LCM_GET_DSI_Lane_num=>dsi:%d\r\n",
			lane_num);
	...
	case LCM_GET_DSI_TE:
	{
		...
		no_printk("fbconfig=>LCM_GET_DSI_TE:%d\n", ret);
		return put_user(ret, (unsigned long *)argp);
	}
	case LCM_GET_DSI_TIMING:
	{
		...
		if (copy_from_user(&timing, (void __user *)argp,
			sizeof(timing))) {
			no_printk("[MIPI_GET_TIMING]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
		ret = PanelMaster_get_dsi_timing(dsi_id, timing.type);
		no_printk("fbconfig=>LCM_GET_DSI_TIMING:%d\n", ret);
		timing.value = ret;
	...
	case MIPI_SET_TIMING:
	{
		...
		if (copy_from_user(&timing, (void __user *)argp,
			sizeof(timing))) {
			no_printk("[MIPI_SET_TIMING]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
	...
	{
		no_printk("[FB_LAYER_GET_EN] not support any more\n");
		return 0;
	}
	case FB_LAYER_GET_INFO:
	{
		no_printk("[FB_LAYER_GET_INFO] not support any more\n");
		return  0;
	}

	case FB_LAYER_DUMP:
	{
		no_printk("[FB_LAYER_DUMP] not support any more\n");
		return  0;
	}
	...
	case LCM_GET_ESD:
	{
		...
		if (copy_from_user(&esd_para, (void __user *)arg,
			sizeof(esd_para))) {
			no_printk("[LCM_GET_ESD]: copy_from_user failed! line:%d\n",
				 __LINE__);
			return -EFAULT;
		}
