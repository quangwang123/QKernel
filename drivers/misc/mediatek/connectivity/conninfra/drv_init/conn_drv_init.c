...
int do_connectivity_driver_init(int chip_id)
{
	...
	i_ret += tmp_ret;
	if (tmp_ret)
		pr_info("Do BT driver init failed, ret:%d\n", tmp_ret);

	tmp_ret = do_gps_drv_init(chip_id);
	i_ret += tmp_ret;
	if (tmp_ret)
		pr_info("Do GPS driver init failed, ret:%d\n", tmp_ret);

	tmp_ret = do_fm_drv_init(chip_id);
	i_ret += tmp_ret;
	if (tmp_ret)
		pr_info("Do FM module init failed, ret:%d\n", tmp_ret);

	tmp_ret = do_wlan_drv_init(chip_id);
	i_ret += tmp_ret;
	if (tmp_ret)
		pr_info("Do wlan module init failed, ret:%d\n", tmp_ret);
