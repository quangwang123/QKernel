...
int conninfra_dev_open(struct inode *inode, struct file *file)
{
	...
	if (!wait_event_timeout(
		...
		if (__ratelimit(&_rs)) {
			pr_info("wait_event_timeout (%d)ms,(%lu)jiffies,return -EIO\n",
				...
				unsigned long event, void *data)
{
	...
	pr_info("conninfra_dev_fb_notifier_callback event=[%u]\n", event);
...
{
	pr_info("conninfra_dev_pwr_on_off_handler start to run\n");
...
static void conninfra_dev_pmic_event_handler(struct work_struct *work)
{
	...
	if (pmic_work) {
		...
		pr_info("[%s] pmic_work is null (id, event)=(%d, %d)", __func__, id, event);
...
static int conninfra_dev_do_drv_init()
{
	...
	if (iret)
		pr_info("init conninfra_test fail, ret = %d\n", iret);
#endif

	iret = conninfra_conf_init();
	if (iret)
		pr_info("init conf fail\n");

	iret = consys_hw_init(&g_conninfra_dev_cb);
	if (iret) {
		pr_info("init consys_hw fail, ret = %d\n", iret);
	...
	if (iret)
		pr_info("Sub driver init fail, iret=%d", iret);
#endif
...
static int conninfra_dev_init(void)
{
	...
	if (iret) {
		pr_info("conninfra_do_drv_init fail, iret = %d", iret);
		return iret;
	}
#endif
	return 0;
err2:
	...
	pr_info("[conninfra_dev_init] err2");
	if (pConninfraClass) {
		class_destroy(pConninfraClass);
		pConninfraClass = NULL;
	}
err1:
	pr_info("[conninfra_dev_init] err1");
	if (cdevErr == 0)
