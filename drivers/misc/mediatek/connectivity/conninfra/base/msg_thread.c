...
do { \
	if (!MSG_OP_EMPTY(prb)) { \
		...
		value = NULL; \
		pr_info("Message queue is empty."); \
	} \
} while (0)
...
static int msg_evt_put_op_to_q(struct msg_op_q *op_q, struct msg_op *op)
{
	...
	if (!op_q || !op) {
		pr_info("invalid input param: pOpQ(0x%p), pLxOp(0x%p)\n", op_q, op);
		return -1;
	}

	ret = osal_lock_sleepable_lock(&op_q->lock);
	if (ret) {
		pr_info("osal_lock_sleepable_lock iRet(%d)\n", ret);
		return -2;
	}

#if defined(CONFIG_MTK_ENG_BUILD) || defined(CONFIG_MT_ENG_BUILD)
	if (msg_evt_opq_has_op(op_q, op)) {
		pr_info("Op(%p) already exists in queue(%p)\n", op, op_q);
		ret = -3;
	}
#endif
...
static struct msg_op *msg_evt_get_op_from_q(struct msg_op_q *op_q)
{
	...
	if (op == NULL) {
		pr_info("MSG_OP_GET(%p) return NULL\n", op_q);
	//osal_opq_dump("FreeOpQ", &g_conninfra_ctx.rFreeOpQ);
		//osal_opq_dump("ActiveOpQ", &g_conninfra_ctx.rActiveOpQ);
	}
...
int msg_evt_put_op_to_active(struct msg_thread_ctx *ctx, struct msg_op *op)
{
	...
	do {
		if (!ctx || !op) {
			pr_info("msg_thread_ctx(0x%p), op(0x%p)\n", ctx, op);
			break;
		}
		...
		if (ret) {
			pr_info("put to active queue fail\n");
		...
		if (wait_ret == 0)
			pr_info("opId(%d) completion timeout\n", op->op.op_id);
		else if (op->result)
...
int msg_evt_opid_handler(struct msg_thread_ctx *ctx, struct msg_op_data *op)
{
	...
	if (opid >= ctx->op_func_size) {
		pr_info("msg_evt_thread invalid OPID(%d)\n", opid);
		return -3;
	}

	if (ctx->op_func[opid] == NULL) {
		pr_info("null handler (%d)\n", opid);
		return -4;
	}
...
static int msg_evt_thread(void *pvData)
{
	...
	for (;;) {
		...
		if (!op) {
			pr_info("get op from activeQ fail\n");
			continue;
		}
		...
		if (ret)
			pr_info("opid (0x%x) failed, ret(%d)\n",
							op->op.op_id, ret);
