...
int osal_thread_create(P_OSAL_THREAD pThread)
{
	...
	if (IS_ERR(task)) {
		pr_info("[%s] create %s thread fail", __func__, pThread->threadName);
		return -1;
	}
...
int _osal_fifo_init(OSAL_FIFO *pFifo, unsigned char *buf, unsigned int size)
{
	...
	if (!pFifo) {
		pr_info("pFifo must be !NULL\n");
		return -1;
	}
	if (pFifo->pFifoBody) {
		pr_info("pFifo->pFifoBody must be NULL\n");
		pr_info("pFifo(0x%p), pFifo->pFifoBody(0x%p)\n",
...
int osal_fifo_init(P_OSAL_FIFO pFifo, unsigned char *buffer, unsigned int size)
{
	...
	if (pFifo->pFifoBody != NULL) {
		pr_info("%s:Because pFifo room is avialable, we clear the room and allocate them again.\n", __func__);
...
void osal_systrace_major_b(const char *fmt, ...)
{
	...
	if (vsnprintf(log, sizeof(log), fmt, args) < 0)
		pr_info("[%s:%d] vsnprintf error", __func__, __LINE__);
	va_end(args);
	osal_systrace_b(log);
}
