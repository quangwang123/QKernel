						...
						const char *pos)
{
	...
	if (size <= 1) {
		pr_info("cfg==> %s has no value assigned\n",
			data->name);
		return -1;
	} else if (size & 0x1) {
		pr_info("cfg==> %s, length should be even\n", data->name);
		return -1;
	}

	ba = (struct conf_byte_ary *)osal_malloc(sizeof(struct conf_byte_ary));
	if (ba == NULL) {
		pr_info("cfg==> %s malloc fail\n", data->name);
		return -1;
	}
	...
	if (buffer == NULL) {
		osal_free(ba);
		pr_info("cfg==> %s malloc fail, size %d\n", data->name, size);
		return -1;
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (osal_strtol(temp, 16, &value) < 0) {
			pr_info("cfg==> %s should be hexadecimal format\n",
						data->name);
			osal_free(ba);
			osal_free(buffer);
			return -1;
...
static int conf_parse_pair(const char *pKey, const char *pVal)
{
	...
	for (i = 0; i < NUM_CFG_FIELDS; i++) {
		...
		if (field->parser(field, pVal)) {
			pr_info("failed to parse %s '%s'.\n", pKey, pVal);
			ret = -1;
		}
		break;
	}
	if (i == NUM_CFG_FIELDS) {
		pr_info("unknown field '%s'.\n", pKey);
		ret = -1;
	}
...
static int conf_parse(const char *pInBuf, unsigned int size)
{
	...
	while ((pLine = osal_strsep(&pch, "\r\n")) != NULL) {
		...
		if (!pVal) {
			pr_info("mal-format cfg string(%s)\n", pLine);
			continue;
		}
	...
	*ppPatch = NULL;
	do {
		ret = request_firmware((const struct firmware **)&fw,
							patch_name, NULL);
		if (ret == -EAGAIN) {
			pr_info("failed to open or read!(%s), retry again!\n",
						patch_name);
			osal_sleep_ms(100);
		}
	} while (ret == -EAGAIN);
	if (ret != 0) {
		pr_info("failed to open or read!(%s)\n", patch_name);
		release_firmware(fw);
		return -1;
	}
	pr_info("loader firmware %s  ok!!\n", patch_name);
	ret = 0;
	*ppPatch = fw;
...
{
	if (name == NULL) {
		pr_info("name is NULL\n");
		return -1;
	}
	if (osal_strlen(name) >= osal_sizeof(g_conninfra_conf.conf_name)) {
		pr_info("name is too long, length=%d, expect to < %zu\n",
				osal_strlen(name),
	...
	pr_info("WMT config file is set to (%s)\n",
				&(g_conninfra_conf.conf_name[0]));
