...
static int conninfra_dbg_ap_reg_read(int par1, int par2, int par3)
{
	...
	if (ap_reg_base) {
		...
		pr_info("AP register ioremap fail!\n");
...
static int conninfra_dbg_connsys_emi_dump(int par1, int par2, int par3)
{
	...
	if (par2 & 0xf) {
		pr_info("EMI dump fail: wrong offset(0x%x), should be 16-byte alignment\n", par2);
		return -1;
	}
	...
	if (buf == NULL) {
		pr_info("[%s] allocate buffer fail\n", __func__);
		return -1;
	}
...
int conninfra_dev_dbg_init(void)
{
	...
	if (g_conninfra_dbg_entry == NULL) {
		pr_info("Unable to create / wmt_aee proc entry\n\r");
		i_ret = -1;
	}
