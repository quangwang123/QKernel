...
static bool conndump_check_cr_readable(struct connsys_dump_ctx* ctx)
{
	...
	if (ctx->callback.reg_readable) {
		...
		pr_info("[%s] reg_readable callback ret = %d\n",
			g_type_name[ctx->conn_type], cb_ret);
		return false;
	}

	pr_info("[%s] reg_readable callback is not implement! Return false\n", g_type_name[ctx->conn_type]);
	return false;
}
...
do { \
	...
	if (!buf) {
		pr_info("Invalid input, buf = %p", buf);
		return 0;
	}
...
format_finish:
	...
	if (ret < 0)
		pr_info("Send issue info to native fail, ret=%d\n", ret);
	return len;
}
	...
	unsigned int buf_len)
{
	...
	if (pDtr != NULL) {
		...
		if (pDtr == NULL) {
			pr_info("Exception substring (%s) not found\n", type_str);
			goto check_task_id;
		}
		pDtr += strlen(type_str);
		pTemp = strchr(pDtr, ',');
		if (pTemp != NULL) {
			...
			if (ret) {
				pr_info("Convert to uint fail, ret=%d, buf=%s\n",
					ret, tempBuf);
			} else {
...
check_task_id:
	...
	pDtr = strstr(pStr, type_str);
	if (pDtr == NULL) {
		pr_info("parser str is NULL,substring(%s)\n", type_str);
		...
		if (pTemp == NULL) {
			pr_info("delimiter( ) is not found,substring(%s)\n",
				type_str);
		} else {
			len = pTemp - pDtr;
			len = (len >= CONNSYS_ASSERT_TYPE_SIZE) ? CONNSYS_ASSERT_TYPE_SIZE - 1 : len;
			...
			ret = kstrtouint(tempBuf, 16, &res);
			if (ret) {
				pr_info("Convert to uint fail, ret=%d\n, buf=%s\n",
					ret, tempBuf);
			} else {
	...
	if (pDtr == NULL) {
		...
		if (pTemp == NULL) {
			...
			ret = kstrtouint(tempBuf, 16, &res);
			if (ret) {
				pr_info("Get fw isr id fail, ret=%d, buf=%s\n", ret, tempBuf);
			} else {
				ctx->info.fw_isr = res;
	...
	if (pDtr == NULL) {
		...
		if (pTemp == NULL) {
			...
			ret = kstrtouint(tempBuf, 16, &res);
			if (ret) {
				pr_info("get fw irq id fail ret=%d, buf=%s\n", ret, tempBuf);
			} else {
				ctx->info.fw_irq = res;
	...
	if (pDtr == NULL) {
		...
		if (pTemp == NULL) {
			...
			if (memcmp(tempBuf, "RB_FULL", strlen("RB_FULL")) == 0) {
				...
				if (pDtr == NULL) {
					pr_info("parser str is NULL,substring(RB_FULL()\n");
				} else {
					pDtr += strlen("RB_FULL(");
					pTemp = strchr(pDtr, ')');
					len = pTemp - pDtr;
...
static int conndump_dump_print_buff(struct connsys_dump_ctx* ctx)
{
	...
	if (buff_len > CONNSYS_DUMP_PRINT_BUFF_SIZE) {
		pr_info("Get print buff idx = %d, but the length is %d\n", buff_len, CONNSYS_DUMP_PRINT_BUFF_SIZE);
		buff_len = CONNSYS_DUMP_PRINT_BUFF_SIZE;
	}
...
static int conndump_dump_dump_buff(struct connsys_dump_ctx* ctx)
{
	...
	if (buff_len > CONNSYS_DUMP_DUMP_BUFF_SIZE) {
		pr_info("Get dump buff idx = %d, but the size is %d\n", buff_len, CONNSYS_DUMP_DUMP_BUFF_SIZE);
		buff_len = CONNSYS_DUMP_DUMP_BUFF_SIZE;
	}
...
#define CR_PER_LINE	11
	...
	if (!conndump_check_cr_readable(ctx)) {
		pr_info("CR not readable, skip cr dump\n");
		return -1;
	}
...
#define MEM_REGION_MAX_COPY_LENGTH	(CONNSYS_DUMP_BUFF_SIZE - MEM_REGION_TAG_LENGTH)
	...
	/* Check reg readable */
	if (!conndump_check_cr_readable(ctx)) {
		pr_info("CR not readable, skip memory region dump\n");
		return -1;
	}
	for (idx = 0; idx < ctx->dump_regions_num; idx++) {
		/* Non-empty name means memory regions  */
		if (ctx->dump_regions[idx].name[0] != 0 &&
			...
			if (!dump_buff) {
				pr_info("Allocate buffer for %s fail.\n",
					ctx->dump_regions[idx].name);
			...
			map_length = conndump_setup_dynamic_remap(
					ctx, ctx->dump_regions[idx].base, ctx->dump_regions[idx].length);
			if (map_length != ctx->dump_regions[idx].length) {
				pr_info("Setup dynamic remap for %s fail. Expect 0x%x but 0x%x",
					ctx->dump_regions[idx].name,
					ctx->dump_regions[idx].length,
					map_length);
				goto next_mem_region;
			...
			if (ret != ctx->dump_regions[idx].length) {
				pr_info("[%s][%s] Send fail, length = 0x%x but only 0x%x send\n",
...
#define EMI_COMMAND_LENGTH	64
	...
	if (ret < 0) {
		pr_info("Start EMI dump fail, ret = %d\n", ret);
		return -1;
	}
...
static int connsys_coredump_init_dump_regions(struct connsys_dump_ctx* ctx, int num)
{
	...
	if (!ctx->dump_regions) {
		...
		pr_info("ctx->dump_regions create fail!\n");
		return 0;
	}
...
int connsys_coredump_setup_dump_region(void* handler)
{
	...
	if (cr_regions_idx & 0x7) {
		pr_info("cr_regions_idx should be multiple of 8. But it is %d.\n", cr_regions_idx);
	}
	...
	if (ret != total_count) {
		pr_info("[%s] allocate %d dump regions failed\n", __func__, total_count);
		return 0;
	}
	...
	struct coredump_event_cb* cb)
{
	...
	/* Get EMI config */
	if (config == NULL) {
		pr_info("Get coredump EMI config fail\n");
		goto error_exit;
	}

	ctx = (struct connsys_dump_ctx*)conndump_malloc(sizeof(struct connsys_dump_ctx));
	if (!ctx) {
		pr_info("Allocate connsys_dump_ctx fail");
		goto error_exit;
	}
	...
	if (!ctx->page_dump_buf) {
		pr_info("Create dump buffer fail.\n");
		goto error_exit;
	}
	...
	if (ctx->emi_virt_addr_base == 0) {
		pr_info("Remap emi fail (0x%08x) size=%d",
