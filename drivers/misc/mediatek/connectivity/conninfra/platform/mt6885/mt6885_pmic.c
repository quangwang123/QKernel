...
//#if CONSYS_PMIC_CTRL_ENABLE
	...
	if (!reg_VCN13)
		pr_info("Regulator_get VCN_13 fail\n");
	else {
...
int consys_pmic_vcn33_1_power_ctl(bool enable, struct regulator *reg_VCN33_1)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			...
			if (ret)
				pr_info("Enable VCN33_1 fail. ret=%d\n", ret);
		}
	} else {
		if (consys_is_rc_mode_enable()) {
			/* Do nothing */
...
int consys_pmic_vcn33_2_power_ctl(bool enable)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			...
			if (ret)
				pr_info("Enable VCN33_2 fail. ret=%d\n", ret);
		}
	} else {
		if (consys_is_rc_mode_enable()) {
			/* Do nothing */
...
int consys_plt_pmic_common_power_ctrl(unsigned int enable)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			...
			if (ret)
				pr_info("Enable VCN18 fail. ret=%d\n", ret);
			...
			if (ret)
				pr_info("Enable VCN13 fail. ret=%d\n", ret);
