...
int consys_conninfra_on_power_ctrl(unsigned int enable)
{
	...
	if (enable) {
		...
		if (check) {
			pr_info("Turn on conn_infra power fail\n");
			return -1;
		}
#else
		...
		if (check != 0)
			pr_info("Check conn_infra_on primary power fail. 0x1000_616C is 0x%08x. Expect [1] as 1.\n",
		...
		if (check != 0)
			pr_info("Check conn_infra_on secondary power fail. 0x1000_6170 is 0x%08x. Expect [1] as 1.\n",
		...
		CONSYS_REG_BIT_POLLING(
			...
			2, 0, 100, 500, check);
		if (check != 0)
			pr_info("Polling AHB RX bus sleep protect turn off fail. status=0x%08x\n",
		...
		CONSYS_REG_BIT_POLLING(
			...
			14, 0, 10, 50, check);
		if (check != 0)
			pr_info("Polling AXI bus sleep protect turn off fail. Status=0x%08x\n",
		...
		CONSYS_REG_BIT_POLLING(
			...
			13, 0, 100, 500, check);
		if (check != 0)
			pr_info("polling AHB TX bus sleep protect turn off fail. Status=0x%08x\n",
				CONSYS_REG_READ(CON_REG_INFRACFG_AO_ADDR + INFRA_TOPAXI_PROTECTEN_STA1_OFFSET));
#endif
		...
		check = consys_platform_spm_conn_ctrl(enable);
		if (check) {
			pr_info("Turn off conn_infra power fail, ret=%d\n", check);
			return -1;
		}
#else
		...
		CONSYS_REG_BIT_POLLING(
			...
			13, 1, 100, 500, check);
		if (check)
			pr_info("Polling AHB TX bus sleep protect turn on fail.\n");
		...
		CONSYS_REG_BIT_POLLING(
			...
			14, 1, 100, 1000, check);
		if (check)
			pr_info("Polling AXI Rx bus sleep protect turn on fail.\n");
		...
		CONSYS_REG_BIT_POLLING(
			...
			2, 1, 10, 1000, check);
		if (check)
			pr_info("Polling AHB RX bus sleep protect turn on fail.\n");
...
int consys_conninfra_wakeup(void)
{
	...
	while (polling_fail_retry > 0) {
		...
		if (check) {
			pr_info("[%s] Check ap2conn slpprot_rdy fail. value=0x%x WAKEUP_TOP=[0x%x]\n",
				__func__,
	...
	if (check != 0) {
		pr_info("[%s] wakeup fail retry %d", __func__, polling_fail_retry);
		return -1;
	}
	...
	if (check != 0)
		pr_info("[%s] Polling chip id fail (0x%x)\n", __func__, consys_hw_ver);
...
int consys_polling_chipid(void)
{
	...
	if (retry == 0) {
		pr_info("Read CONSYS version id fail. Expect 0x%x but get 0x%x\n",
...
static int connsys_a_die_efuse_read(unsigned int efuse_addr)
{
	...
	if (consys_sema_acquire_timeout(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[EFUSE READ] Require semaphore fail\n");
		return 0;
	}
	...
	if (((ret & (0x1 << 29)) >> 29) == 1) {
		...
		if (ret0 || ret1 || ret2 || ret3)
			pr_info("efuse read error: [%d, %d, %d, %d]", ret0, ret1, ret2, ret3);
		ret = 1;
	} else {
		connsys_a_die_thermal_cal(0, 0);
		pr_info("EFUSE is invalid\n");
		ret = 0;
	}
...
static int connsys_bt_low_power_setting(bool bt_only)
{
	...
	if (bt_only) {
		...
		if (hw_version == 0x66358A00) {
			...
			pr_info("[%s] wrong adie version (0x%08x)\n", __func__, hw_version);
	} else {
		if (hw_version == 0x66358A00) {
...
void connsys_debug_select_config(void)
{
			...
			0x1, 0x7);
	{
		...
		if (vir_addr) {
			...
			pr_info("remap 0x1800_6000 fail\n");
		}
	}
#endif
...
int consys_sema_acquire_timeout(unsigned int index, unsigned int usec)
{
	...
	if (index >= CONN_SEMA_NUM_MAX) {
		pr_info("[%s] wrong parameter: %d", __func__, index);
		return CONN_SEMA_GET_FAIL;
	}
	...
	pr_info("Get semaphore 0x%x timeout, dump status:\n", index);
	pr_info("M0:[0x%x] M1:[0x%x] M2:[0x%x] M3:[0x%x]\n",
...
static int consys_spi_read_nolock(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	...
	if (check != 0) {
		pr_info("[%s][%d][STEP1] polling 0x%08x bit %d fail. Value=0x%08x\n",
	...
	CONSYS_REG_BIT_POLLING(
		...
		op->polling_bit, 0, 100, 50, check);
	if (check != 0) {
		pr_info("[%s][%d][STEP4] polling 0x%08x bit %d fail. Value=0x%08x\n",
			__func__, subsystem, CONN_REG_RFSPI_ADDR + op->busy_cr,
			op->polling_bit,
			CONSYS_REG_READ(CONN_REG_RFSPI_ADDR + op->busy_cr));
		return CONNINFRA_SPI_OP_FAIL;
...
int consys_spi_read(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	...
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[SPI READ] Require semaphore fail\n");
		return CONNINFRA_SPI_OP_FAIL;
	}
...
int consys_spi_write(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	...
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[SPI WRITE] Require semaphore fail\n");
		return CONNINFRA_SPI_OP_FAIL;
	}
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	ret = consys_spi_read_nolock(subsystem, addr, &data);
	if (ret) {
		pr_info("[%s][%s] Get 0x%08x error, ret=%d",
...
int consys_adie_top_ck_en_on(enum consys_adie_ctl_type type)
{
	...
	if (consys_sema_acquire_timeout(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[%s][%s] acquire semaphore (%d) timeout\n",
...
int consys_adie_top_ck_en_off(enum consys_adie_ctl_type type)
{
	...
	if (consys_sema_acquire_timeout(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[%s][%s] acquire semaphoreaore (%d) timeout\n",
			__func__, get_adie_ctrl_type_str(type), CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX);
		return -1;
	}
...
#define MAX_SPI_CLOCK_SWITCH_COUNT	100
	...
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[SPI CLOCK SWITCH] Require semaphore fail\n");
		return -1;
	}
	...
	if (type == CONNSYS_SPI_SPEED_26M) {
		...
		if (counter == MAX_SPI_CLOCK_SWITCH_COUNT) {
			pr_info("[%s] switch to 64M fail\n", __func__);
			ret = -1;
		}
	} else {
		ret = -1;
		pr_info("[%s] wrong parameter %d\n", __func__, type);
	}
...
int consys_subsys_status_update(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[%s] acquire semaphore (%d) timeout\n",
