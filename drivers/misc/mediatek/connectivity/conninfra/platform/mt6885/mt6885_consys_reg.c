...
static int consys_is_bus_hang(void)
{
	...
	if ((r & 0x1) != 0x1) {
		pr_info("conninfra off bus might hang cirq=[0x%08x]", r);
		consys_bus_hang_dump_c();
...
unsigned long consys_reg_validate_idx_n_offset(unsigned int idx, unsigned long offset)
{
	...
	if (idx < 0 || idx >= CONSYS_BASE_ADDR_MAX) {
		pr_info("ConsysReg failed: No support the base %d\n", idx);
		return 0;
	}
	...
	if (res == 0) {
		pr_info("ConsysReg failed: No support the base idx is 0 index=[%d]\n", idx);
		return 0;
	}

	if (offset >= conn_reg.reg_base_addr[idx].size) {
		pr_info("ConnReg failed: index(%d), offset(%d) over max size(%llu) %s\n",
...
int consys_reg_init(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			if (ret) {
				pr_info("Get Reg Index(%d-%s) failed",
		...
		pr_info("[%s] can't find CONSYS compatible node\n", __func__);
		return ret;
	}
	return 0;
