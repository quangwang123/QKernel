...
int consys_clk_get_from_dts(struct platform_device *pdev)
{
	...
	if (IS_ERR(clk_scp_conn_main)) {
		pr_info("[CCF]cannot get clk_scp_conn_main clock.\n");
		return PTR_ERR(clk_scp_conn_main);
	}
	pr_info("[CCF]clk_scp_conn_main=%p\n", clk_scp_conn_main);
...
int consys_platform_spm_conn_ctrl(unsigned int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_info("Turn on oonn_infra power fail. Ret=%d\n", ret);
			return -1;
		}
	} else {
...
int consys_co_clock_type(void)
{
	...
	if (NULL == conf) {
		pr_info("[%s] Get conf fail", __func__);
		return -1;
	}
...
#define TEMP_SIZE		13
	...
	if (addr == NULL) {
		pr_info("GPT2_CTRL_BASE remap fail");
		return -1;
	}
	...
	if (consys_sema_acquire_timeout(CONN_SEMA_THERMAL_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_info("[THERM QRY] Require semaphore fail\n");
