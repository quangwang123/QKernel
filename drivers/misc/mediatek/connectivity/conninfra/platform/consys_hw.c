...
int consys_hw_get_clock_schematic(void)
{
	if (consys_hw_ops->consys_plt_co_clock_type)
		...
		pr_info("consys_hw_ops->consys_co_clock_type not supported\n");
...
unsigned int consys_hw_chipid_get(void)
{
	if (consys_hw_ops->consys_plt_soc_chipid_get)
		...
		pr_info("consys_plt_soc_chipid_get not supported\n");
...
int mtk_conninfra_probe(struct platform_device *pdev)
{
	...
	if (consys_reg_mng_init(pdev) != 0) {
		pr_info("consys_plt_read_reg_from_dts fail");
		return -1;
	}
	...
	if (consys_hw_ops->consys_plt_clk_get_from_dts)
		...
		pr_info("consys_plt_clk_get_from_dtsfail");
		return -2;
	}
	...
	ret = emi_mng_init();
	if (ret) {
		pr_info("emi_mng init fail, %d\n", ret);
		return -3;
	}

	ret = pmic_mng_init(pdev, g_conninfra_dev_cb);
	if (ret) {
		pr_info("pmic_mng init fail, %d\n", ret);
		return -4;
	}
	...
	if (emi_info) {
		...
		pr_info("Connsys log didn't init because EMI is invalid\n");
	}
...
int consys_hw_init(struct conninfra_dev_cb *dev_cb)
{
	...
	if (iRet)
		pr_info("Conninfra platform driver registered failed(%d)\n", iRet);
