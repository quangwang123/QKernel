...
static VOID stp_dbg_core_dump_timeout_handler(timer_handler_arg arg)
{
	...
	STP_DBG_pr_info(" coredump timer timeout, coredump maybe not finished successfully\n");
}
...
static VOID stp_dbg_dump_emi_timeout_handler(timer_handler_arg arg)
{
	STP_DBG_pr_info("dump emi timeout!\n");
...
static _osal_inline_ P_WCN_CORE_DUMP_T stp_dbg_core_dump_init(UINT32 timeout)
{
	...
	if (!core_dmp) {
		STP_DBG_pr_info("alloc mem failed!\n");
		return NULL;
	}
...
static UINT32 stp_dbg_core_dump_header_init(P_WCN_CORE_DUMP_T dmp)
{
	...
	if (dmp->p_head == NULL) {
		...
		if (dmp->p_head == NULL) {
			STP_DBG_pr_info("alloc memory for head information failed\n");
			return -1;
		}
	}
	if (dmp->p_head != NULL)
...
static _osal_inline_ INT32 stp_dbg_core_dump_in(P_WCN_CORE_DUMP_T dmp, PUINT8 buf, INT32 len)
{
	...
	if ((!dmp) || (!buf)) {
		STP_DBG_pr_info("invalid pointer!\n");
		return -1;
	}

	ret = osal_lock_sleepable_lock(&dmp->dmp_lock);
	if (ret) {
		STP_DBG_pr_info("--->lock dmp->dmp_lock failed, ret=%d\n", ret);
		return ret;
	}
...
#define INFO_HEAD ";CONSYS FW CORE, "
	...
	if ((dmp->p_head != NULL)
		...
		if (ret) {
			STP_DBG_pr_info("set fw issue infor fail(%d),maybe fw warm reset...\n",
					 ret);
...
static _osal_inline_ INT32 stp_dbg_core_dump_nl(P_WCN_CORE_DUMP_T dmp, PUINT8 buf, INT32 len)
{
	...
	switch (dmp->sm) {
	case CORE_DUMP_INIT:
		STP_DBG_pr_info("CONSYS coredump start, please wait up to %d minutes.\n",
...
{
	if (!pbuf) {
		STP_DBG_pr_info("Parameter error\n");
		return -1;
	}

	if (mtk_wcn_stp_coredump_start_get()) {
		STP_DBG_pr_info("assert has been triggered\n");
		return -1;
	}
				 ...
				 PINT32 out_sz, INT32 finish)
{
	...
	if (!stream) {
		STP_DBG_pr_info("invalid workspace!\n");
		return -1;
	}
		...
		if (finish) {
			while (1) {
				...
				if (val == Z_OK)
					...
					break;
				STP_DBG_pr_info("finish operation failed %d\n", val);
				return -3;
			}
		}
		...
		INT32 L2_buf_sz)
{
	...
	if (!compress) {
		STP_DBG_pr_info("alloc compressor failed!\n");
		goto fail;
	}
	...
	if (compress->compress_type == GZIP) {
		...
		if (!compress->worker) {
			STP_DBG_pr_info("alloc stream failed!\n");
			goto fail;
		}
		...
		if (!pstream->workspace) {
			STP_DBG_pr_info("alloc workspace failed!\n");
			goto fail;
		}
	...
	if (!compress->L1_buf) {
		STP_DBG_pr_info("alloc %d bytes for L1 buf failed!\n", compress->L1_buf_sz);
		goto fail;
	}

	compress->L2_buf = osal_malloc(compress->L2_buf_sz);
	if (!compress->L2_buf) {
		STP_DBG_pr_info("alloc %d bytes for L2 buf failed!\n", compress->L2_buf_sz);
		goto fail;
	}
		...
		INT32 is_iobuf, INT32 finish)
{
	...
	if (len > (cprs->L1_buf_sz - cprs->L1_pos)) {
		...
		if (cprs->f_compress_en && cprs->handler) {
			...
			if (!ret) {
				...
				if (cprs->L2_pos >= cprs->L2_buf_sz)
					STP_DBG_pr_info("coredump size too large(%d), L2 buf overflow\n",
					cprs->L2_pos);
				...
				STP_DBG_pr_info("compress error!\n");
		} else {
			/* no need compress */
		...
		if (len > cprs->L1_buf_sz) {
			STP_DBG_pr_info("len=%d, too long err!\n", len);
		} else {
...
INT32 stp_dbg_dmp_append(MTKSTP_DBG_T *stp_dbg, PUINT8 pBuf, INT32 max_len)
{
	...
	if (!pBuf || max_len < 8) { /* 8: length of "<!---->\n" */
		STP_DBG_pr_info("invalid param, pBuf:%p, max_len:%d\n", pBuf, max_len);
		return 0;
	}
			      ...
			      INT32 crc, INT32 dir, INT32 len, INT32 dbg_type)
{
	...
	if (!hdr) {
		STP_DBG_pr_info("function invalid\n");
		return -EINVAL;
	}
...
static _osal_inline_ INT32 stp_dbg_add_pkt(MTKSTP_DBG_T *stp_dbg, STP_DBG_HDR_T *hdr, const PUINT8 body)
{
	...
	if (hdr->dbg_type == STP_DBG_FW_DMP) {
		if (hdr->last_dbg_type != STP_DBG_FW_DMP) {
			...
			if (!avl_num)
				STP_DBG_pr_info("there is no avl entry stp_dbg logsys!!!\n");
		}
	}
...
VOID stp_dbg_nl_init(VOID)
{
	...
	if (genl_register_family(&stp_dbg_gnl_family) != 0) {
		STP_DBG_pr_info("%s(): GE_NELINK family registration fail\n", __func__);
	} else {
		if (genl_register_ops(&stp_dbg_gnl_family, &stp_dbg_gnl_ops_bind) != 0)
			STP_DBG_pr_info("%s(): BIND operation registration fail\n", __func__);

		if (genl_register_ops(&stp_dbg_gnl_family, &stp_dbg_gnl_ops_reset) != 0)
			STP_DBG_pr_info("%s(): RESET operation registration fail\n", __func__);
...
static INT32 stp_dbg_nl_bind(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (i == MAX_BIND_PROCESS) {
		STP_DBG_pr_info("%s(): exceeding binding limit %d\n", __func__, MAX_BIND_PROCESS);
...
static INT32 stp_dbg_nl_reset(struct sk_buff *skb, struct genl_info *info)
{
	STP_DBG_pr_info("%s(): should not be invoked\n", __func__);
...
INT32 stp_dbg_nl_send(PINT8 aucMsg, UINT8 cmd, INT32 len)
{
	...
	if (num_bind_process == 0) {
		...
		STP_DBG_pr_info("%s(): the process is not invoked\n", __func__);
		return 0;
	}
	...
	for (i = 0; i < num_bind_process; i++) {
		...
		if (skb) {
			...
			if (msg_head == NULL) {
				nlmsg_free(skb);
				STP_DBG_pr_info("%s(): genlmsg_put fail...\n", __func__);
				return -1;
			}
			...
			if (rc != 0) {
				nlmsg_free(skb);
				STP_DBG_pr_info("%s(): nla_put_string fail...: %d\n", __func__, rc);
				return rc;
			}
			...
			STP_DBG_pr_info("%s(): genlmsg_new fail...\n", __func__);
		}
	}
...
INT32 stp_dbg_dump_send_retry_handler(PINT8 tmp, INT32 len)
{
	...
	while (ret) {
		nl_retry++;
		if (ret == 32) {
			STP_DBG_pr_info("**dump send timeout : %d**\n", ret);
			ret = 1;
			break;
		}
		if (nl_retry > 1000) {
			STP_DBG_pr_info("**dump send fails, and retry more than 1000: %d.**\n", ret);
			ret = 2;
			break;
		}
...
#define PKT_MULTIPLIER 1
	...
	if (g_core_dump->count == 0) {
		...
		if (!g_core_dump->compressor) {
			STP_DBG_pr_info("create compressor failed!\n");
			stp_dbg_compressor_deinit(g_core_dump->compressor);
			return -1;
		}
	}
...
#define WDT_INFO_HEAD "Watch Dog Timeout"
	...
	if (!str) {
		STP_DBG_pr_info("NULL string source\n");
		return -1;
	}

	if (!g_stp_dbg_cpupcr) {
		STP_DBG_pr_info("NULL pointer\n");
		return -2;
	}
	...
	case STP_DBG_ASSERT_INFO:
		...
		if (pDtr != NULL) {
			...
			STP_DBG_pr_info("parser str is NULL,substring(%s)\n", parser_sub_string[type]);
			return -3;
		}

		if (pTemp == NULL) {
			STP_DBG_pr_info("delimiter( ) is not found,substring(%s)\n",
					 parser_sub_string[type]);
			return -4;
		}
		...
		pTemp = osal_strchr(pDtr, '#');
		if (pTemp == NULL) {
			STP_DBG_pr_info("parser '#' is not find\n");
			return -5;
		}
		pTemp += 1;
	...
	case STP_DBG_FW_TASK_ID:
		...
		if (ret) {
			STP_DBG_pr_info("get fw task id fail(%d)\n", ret);
			return -4;
		}
	...
	case STP_DBG_FW_ISR:
		...
		ret = osal_strtol(tempBuf, 16, &res);
		if (ret) {
			STP_DBG_pr_info("get fw isr id fail(%d)\n", ret);
			return -4;
		}
	...
	case STP_DBG_FW_IRQ:
		...
		ret = osal_strtol(tempBuf, 16, &res);
		if (ret) {
			STP_DBG_pr_info("get fw irq id fail(%d)\n", ret);
			return -4;
		}
	...
	case STP_DBG_ASSERT_TYPE:
		...
		if (osal_memcmp(tempBuf, "RB_FULL", osal_strlen("RB_FULL")) == 0) {
			...
			if (pDtr != NULL) {
				...
				STP_DBG_pr_info("parser str is NULL,substring(RB_FULL()\n");
				return -5;
			}
			len = pTemp - pDtr;
			len = (len >= STP_ASSERT_TYPE_SIZE) ? STP_ASSERT_TYPE_SIZE - 1 : len;
		...
		break;
	default:
		STP_DBG_pr_info("unknown parser type\n");
		break;
	}
...
static _osal_inline_ P_STP_DBG_CPUPCR_T stp_dbg_cpupcr_init(VOID)
{
	...
	if (!pSdCpupcr) {
		STP_DBG_pr_info("stp dbg cpupcr allocate memory fail!\n");
		return NULL;
	}
...
static _osal_inline_ P_STP_DBG_DMAREGS_T stp_dbg_dmaregs_init(VOID)
{
	...
	if (!pDmaRegs) {
		STP_DBG_pr_info("stp dbg dmareg allocate memory fail!\n");
		return NULL;
	}
...
INT32 stp_dbg_poll_cpupcr(UINT32 times, UINT32 sleep, UINT32 cmd)
{
	...
	if (!g_stp_dbg_cpupcr) {
		STP_DBG_pr_info("NULL reference pointer\n");
		return -1;
	}
	...
	if (cmd) {
		...
		if (chip_id == 0x6632) {
			for (i = 0; i < 8; i++) {
				...
				if (i_ret)
					STP_DBG_pr_info("read CCCR fail(%d), address(0x%x)\n",
							i_ret, CCCR_F8 + i);
				else
...
INT32 stp_dbg_poll_dmaregs(UINT32 times, UINT32 sleep)
{
	...
	if (g_stp_dbg_dmaregs->count + times > STP_DBG_DMAREGS_NUM) {
		if (g_stp_dbg_dmaregs->count > STP_DBG_DMAREGS_NUM) {
			STP_DBG_pr_info("g_stp_dbg_dmaregs->count:%d must less than STP_DBG_DMAREGS_NUM:%d\n",
				g_stp_dbg_dmaregs->count, STP_DBG_DMAREGS_NUM);
			g_stp_dbg_dmaregs->count = 0;
			STP_DBG_pr_info("g_stp_dbg_dmaregs->count be set default value 0\n");
		}
		times = STP_DBG_DMAREGS_NUM - g_stp_dbg_dmaregs->count;
	}
	if (times > STP_DBG_DMAREGS_NUM) {
		STP_DBG_pr_info("times overflow, set default value:0\n");
		times = 0;
	}
...
INT32 stp_dbg_set_fw_info(PUINT8 issue_info, UINT32 len, ENUM_STP_FW_ISSUE_TYPE issue_type)
{
	...
	if ((issue_type == STP_FW_ASSERT_ISSUE) ||
		...
		STP_DBG_pr_info("invalid issue type(%d)\n", issue_type);
		return -3;
	}
...
INT32 stp_dbg_cpupcr_infor_format(PUINT8 buf, UINT32 max_len)
{
	...
	/* never retrun negative value */
	if (!g_stp_dbg_cpupcr || !buf) {
		STP_DBG_pr_info("NULL pointer, g_stp_dbg_cpupcr:%p, buf:%p\n",
				 g_stp_dbg_cpupcr, buf);
		return 0;
	}
...
MTKSTP_DBG_T *stp_dbg_init(PVOID btm_half)
{
	...
	if (IS_ERR(stp_dbg->logsys)) {
		STP_DBG_pr_info("-ENOMEM stp_gdb->logsys\n");
		goto ERR_EXIT2;
	}
	memset(stp_dbg->logsys, 0, sizeof(MTKSTP_LOG_SYS_T));
	...
	g_core_dump = stp_dbg_core_dump_init(STP_CORE_DUMP_TIMEOUT);
	if (!g_core_dump) {
		STP_DBG_pr_info("-ENOMEM wcn_coer_dump_init fail!");
		goto ERR_EXIT2;
	}
	g_stp_dbg_cpupcr = stp_dbg_cpupcr_init();
	if (!g_stp_dbg_cpupcr) {
		STP_DBG_pr_info("-ENOMEM stp_dbg_cpupcr_init fail!");
		goto ERR_EXIT2;
	}
	g_stp_dbg_dmaregs = stp_dbg_dmaregs_init();
	if (!g_stp_dbg_dmaregs) {
		STP_DBG_pr_info("-ENOMEM stp_dbg_dmaregs_init fail!");
		goto ERR_EXIT2;
	}
	return stp_dbg;
...
INT32 stp_dbg_stop_emi_dump(VOID)
{
	...
	if (mtk_wcn_stp_emi_dump_flag_get() == 1) {
		STP_DBG_pr_info("stopping emi dump!\n");
		return -2;
	}
