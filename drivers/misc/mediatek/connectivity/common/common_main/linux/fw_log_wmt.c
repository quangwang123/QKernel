	...
	unsigned long arg)
{
	...
	case WMT_FW_LOG_IOCTL_ON_OFF:
		pr_info("ioctl: WMT_FW_LOG_IOCTL_ON_OFF(%lu)", arg);
		if (arg == 0 || arg == 1)
			wmt_lib_fw_log_ctrl(WMT_FWLOG_MCU, (unsigned char)arg, 0xFF);
		break;
	case WMT_FW_LOG_IOCTL_SET_LEVEL:
		pr_info("ioctl: WMT_FW_LOG_IOCTL_SET_LEVEL(%lu)", arg);
		if (arg <= 4)
...
int fw_log_wmt_init(void)
{
	...
	if (ret)
		pr_info("fail to connsys_log_init\n");

	ret = connsys_log_register_event_cb(CONNLOG_TYPE_MCU,
		fw_log_wmt_event_cb);
	if (ret)
		pr_info("fail to connsys_log_register_event_cb\n");
...
void fw_log_wmt_deinit(void)
{
	...
	cdev_del(&gLogCdev);
	unregister_chrdev_region(gDevId, 1);
	pr_info("fw_log_wmt_driver_deinit done\n");
}
