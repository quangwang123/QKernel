...
static _osal_inline_ INT32 stp_dbg_soc_paged_dump(INT32 dump_sink)
{
	...
	if (chip_reset_only) {
		STP_DBG_pr_info("is chip reset only\n");
		ret = -3;
		return ret;
	}
	...
	if (dump_num != 0) {
		...
		STP_DBG_pr_info("can not get consys dump num and default num is %d\n", CORE_DUMP_NUM);
	}
	...
	do {
...
#if !WMT_DBG_SUPPORT
				STP_DBG_pr_info("Wait Timeout: %llu > %d\n", elapsed_time, EMI_SYNC_TIMEOUT);
		...
		if (!dump_phy_addr) {
			STP_DBG_pr_info("get paged dump phy address fail\n");
			ret = -1;
			break;
		}
		...
		switch (dump_sink) {
			case 1:
				...
				if (ret == 0)
					...
					STP_DBG_pr_info("aee send timeout!\n");
					abort = 1;
					goto paged_dump_end;
				} else
			...
			case 2:
				...
				if (ret == 0)
					...
					STP_DBG_pr_info("dump send timeout!\n");
					abort = 1;
					goto paged_dump_end;
				} else
					STP_DBG_pr_info("dump send error!\n");
				break;
			default:
...
paged_dump_end:
		...
		if (wmt_plat_get_dump_info(p_ecsi->p_ecso->emi_apmem_ctrl_chip_paded_dump_end) ||
			...
			STP_DBG_pr_info("paged dump fail, generate fake coredump message\n");
...
static _osal_inline_ INT32 stp_dbg_soc_paged_trace(VOID)
{
	...
	do {
		...
		if (loop_cnt1 >= 10) {
			STP_DBG_pr_info("polling CTRL STATE fail\n");
			ret = -1;
			break;
		}
		...
		if (!dump_vir_addr) {
			STP_DBG_pr_info("get vir dump address fail\n");
			ret = -2;
			break;
		}
...
INT32 stp_dbg_soc_core_dump(INT32 dump_sink)
{
	...
	if (dump_sink == 0 || chip_reset_only == 1) {
		...
		if (ret)
			STP_DBG_pr_info("stp_dbg_soc_paged_trace fail: %d!\n", ret);
...
PUINT8 stp_dbg_soc_id_to_task(UINT32 id)
{
	...
	if (id >= STP_DBG_TASK_ID_MAX) {
		STP_DBG_pr_info("task id(%d) overflow(%d)\n", id, STP_DBG_TASK_ID_MAX);
		return NULL;
	}
