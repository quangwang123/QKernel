...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (iRet)
			WMT_PLAT_pr_info("conn_power_on fail(%d)\n", iRet);
		...
		if (iRet)
			WMT_PLAT_pr_info("conn_power_off fail(%d)\n", iRet);
...
static INT32 consys_ahb_clock_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (iRet)
			WMT_PLAT_pr_info("clk_prepare_enable(clk_infra_conn_main) fail(%d)\n", iRet);
...
static INT32 polling_consys_chipid(VOID)
{
	...
	if ((0 == retry) || (0 == consysHwChipId)) {
		WMT_PLAT_pr_info("Maybe has a consys power on issue,(0x%08x)\n", consysHwChipId);
	...
	if (enable) {
		...
		if (reg_VCN28) {
			...
			if (regulator_enable(reg_VCN28))
				WMT_PLAT_pr_info("enable VCN_2V8 fail!\n");
			else
		...
		if (reg_VCN28) {
			if (regulator_disable(reg_VCN28))
				WMT_PLAT_pr_info("disable VCN_2V8 fail!\n");
			else
...
#else
	WMT_PLAT_pr_info("not define platform config\n");
#endif
#endif
	return 0;
}
...
static INT32 consys_clk_get_from_dts(struct platform_device *pdev)
{
	...
	if (IS_ERR(clk_infra_conn_main)) {
		WMT_PLAT_pr_info("[CCF]cannot get clk_infra_conn_main clock.\n");
	...
	if (node) {
		*irq_num = irq_of_parse_and_map(node, 0);
		...
		if (of_property_read_u32_array(node, "interrupts", irq_info, ARRAY_SIZE(irq_info))) {
			WMT_PLAT_pr_info("get irq flags from DTS fail!!\n");
			return iret;
		}
		*irq_flag = irq_info[2];
