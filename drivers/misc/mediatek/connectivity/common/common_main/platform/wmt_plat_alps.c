...
/* #error "need to disable EINT here" */
	...
	if (wmt_plat_bgf_irq_cb != NULL)
		...
		WMT_PLAT_pr_info("WMT-PLAT: wmt_plat_bgf_irq_cb not registered\n");
}
#endif
	...
	case PIN_BGF_EINT:
		if (state == PIN_STA_INIT) {
			if (wmt_detect_get_chip_type() == WMT_CHIP_TYPE_SOC) {
				...
				if (iret) {
					WMT_PLAT_pr_info("request_irq fail,irq_no(%d),iret(%d)\n",
							  bgf_irq_num, iret);
					return iret;
				} else {
					iret = enable_irq_wake(bgf_irq_num);
					if (iret)
						WMT_PLAT_pr_info("enable irq wake fail,irq_no(%d),iret(%d)\n",
							bgf_irq_num, iret);
					iret = 0;
				}
			} else {
...
{
	WMT_PLAT_pr_info("host i2s pin not defined!!!\n");
...
{
	WMT_PLAT_pr_info("host gps sync pin not defined!!!\n");
...
static INT32 wmt_plat_soc_gps_lna_ctrl(ENUM_PIN_STATE state)
{
	...
	case PIN_STA_OUT_L:
		...
		break;
	default:
		WMT_PLAT_pr_info("%d mode not defined for  gps lna pin !!!\n", state);
		break;
	}
#else
...
INT32 wmt_plat_soc_paldo_ctrl(ENUM_PALDO_TYPE ePt, ENUM_PALDO_OP ePo)
{
	...
	case EFUSE_PALDO:
		...
		break;
	default:
		WMT_PLAT_pr_info("WMT-PLAT:Warnning, invalid type(%d) in palod_ctrl\n", ePt);
		break;
	}
...
UINT32 wmt_plat_force_trigger_assert(ENUM_FORCE_TRG_ASSERT_T type)
{
	...
	case STP_FORCE_TRG_ASSERT_DEBUG_PIN:
		...
		break;
	default:
		WMT_PLAT_pr_info("unknown force trigger assert type\n");
		break;
	}
