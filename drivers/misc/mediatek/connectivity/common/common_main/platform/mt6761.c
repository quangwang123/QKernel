...
INT32 mtk_wcn_consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		if (remap_addr3 == 0) {
			WMT_PLAT_pr_info("remap jtag_addr3 fail!\n");
			ret =  -1;
			goto error;
		}
...
static VOID consys_afe_reg_setting(VOID)
{
	...
	if (consys_afe_reg_base) {
		...
		WMT_PLAT_pr_info("AFE base(0x%x) ioremap fail!\n", CONSYS_AFE_REG_BASE);
#endif
}
...
static VOID consys_clock_fail_dump(VOID)
{
	WMT_PLAT_pr_info("CONN_HIF_TOP_MISC=0x%08x CONN_HIF_BUSY_STATUS=0x%08x\n",
	...
	WMT_PLAT_pr_info("Write CONSYS_HIF_DBG_IDX to 0x3333\n");

	WMT_PLAT_pr_info("CONSYS_HIF_DBG_PROBE=0x%08x CONN_HIF_TOP_MISC=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_HIF_DBG_PROBE),
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_HIF_TOP_MISC));
	WMT_PLAT_pr_info("CONN_HIF_BUSY_STATUS=0x%08x CONN_HIF_PDMA_BUSY_STATUS=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_HIF_BUSY_STATUS),
	...
	WMT_PLAT_pr_info("Write CONSYS_HIF_DBG_IDX to 0x2222\n");

	WMT_PLAT_pr_info("CONSYS_HIF_DBG_PROBE=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_HIF_DBG_PROBE));
	...
	WMT_PLAT_pr_info("Write CONSYS_HIF_DBG_IDX to 0x4444\n");

	WMT_PLAT_pr_info("CONSYS_HIF_DBG_PROBE=0x%08x CONN_MCU_EMI_CONTROL=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_HIF_DBG_PROBE),
		CONSYS_REG_READ(conn_reg.mcu_conn_hif_pdma_base + CONSYS_SW_IRQ_OFFSET));
	WMT_PLAT_pr_info("CONN_MCU_CLOCK_CONTROL=0x%08x CONN_MCU_BUS_CONTROL=0x%08x\n",
	...
	WMT_PLAT_pr_info("Write CONSYS_DEBUG_SELECT to 0x003e3d00\n");

	WMT_PLAT_pr_info("CONN_MCU_DEBUG_STATUS=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DEBUG_STATUS));

	CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_DEBUG_SELECT, 0x00403f00);
	WMT_PLAT_pr_info("Write CONSYS_DEBUG_SELECT to 0x00403f00\n");

	WMT_PLAT_pr_info("CONN_MCU_DEBUG_STATUS=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DEBUG_STATUS));

	CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_DEBUG_SELECT, 0x00424100);
	WMT_PLAT_pr_info("Write CONSYS_DEBUG_SELECT to 0x00424100\n");

	WMT_PLAT_pr_info("CONN_MCU_DEBUG_STATUS=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DEBUG_STATUS));

	CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_DEBUG_SELECT, 0x00444300);
	WMT_PLAT_pr_info("Write CONSYS_DEBUG_SELECT to 0x00444300\n");

	WMT_PLAT_pr_info("CONN_MCU_DEBUG_STATUS=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_DEBUG_STATUS));

	WMT_PLAT_pr_info("CONN_ON_HOST_CSR_MISC=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + CONN_ON_HOST_CSR_MISC));

	WMT_PLAT_pr_info("CONN_ON_IRQ_CTL=0x%08x\n",
		CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + CONN_ON_IRQ_CTL));

	WMT_PLAT_pr_info("CONN_ON_IRQ_STATUS=0x%08x\n",
