...
{
	WMT_PLAT_pr_info("Does not support on combo\n");
	return 0;
}
...
static INT32 mtk_wmt_probe(struct platform_device *pdev)
{
	...
	if (gConEmiPhyBase) {
		...
		WMT_PLAT_pr_info("consys emi memory address gConEmiPhyBase invalid\n");
	}
	...
	if (IS_ERR(consys_pinctrl)) {
		WMT_PLAT_pr_info("cannot find consys pinctrl.\n");
		consys_pinctrl = NULL;
	}
...
INT32 mtk_wcn_consys_hw_pwr_on(UINT32 co_clock_type)
{
	...
	if (!gConEmiPhyBase) {
		WMT_PLAT_pr_info("EMI base address is invalid, CONNSYS can not be powered on!");
		return -WMT_ERRCODE_EMI_NOT_READY;
	}
	iRet = mtk_wcn_consys_hw_reg_ctrl(1, co_clock_type);
	if (iRet) {
		WMT_PLAT_pr_info("CONSYS-HW-PWR-ON, failed!(%d)\n", iRet);
		return iRet;
	}
...
INT32 mtk_wcn_consys_hw_pwr_off(UINT32 co_clock_type)
{
	...
	if (iRet) {
		WMT_PLAT_pr_info("CONSYS-HW-PWR-OFF, failed!(%d)\n", iRet);
		return iRet;
	}
...
INT32 mtk_wcn_consys_hw_init(VOID)
{
	...
	if (iRet)
		WMT_PLAT_pr_info("WMT platform driver registered failed(%d)\n", iRet);
	else {
...
PUINT8 mtk_wcn_consys_emi_virt_addr_get(UINT32 ctrl_state_offset)
{
	...
	if (!pEmibaseaddr) {
		WMT_PLAT_pr_info("EMI base address is NULL\n");
		return NULL;
	}
...
INT32 mtk_wcn_consys_set_dbg_mode(UINT32 flag)
{
	...
	if (!vir_addr) {
		WMT_PLAT_pr_info("get vir address fail\n");
		return -2;
	}
	if (flag) {
		ret = 0;
		...
		PUINT32 value)
{
	...
	if (!reg_info) {
		WMT_PLAT_pr_info("reg_info osal_malloc fail\n");
		goto fail;
	}
	...
	if (node) {
		if (of_property_read_u32_array(node, "reg", reg_info, reg_info_size)) {
			WMT_PLAT_pr_info("get reg from DTS fail!!\n");
			goto fail;
		}
	} else {
	...
	if (reg_info[index*4 + 3] < offset) {
		WMT_PLAT_pr_info("Access overflow of address(0x%x), offset(0x%x)!\n",
