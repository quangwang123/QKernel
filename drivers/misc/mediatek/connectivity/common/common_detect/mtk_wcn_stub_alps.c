...
int mtk_wcn_cmb_stub_reg(struct _CMB_STUB_CB_ *p_stub_cb)
{
	...
	if ((!p_stub_cb)
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] invalid p_stub_cb:0x%p size(%d)\n",
...
int mtk_wcn_cmb_stub_aif_ctrl(enum CMB_STUB_AIF_X state, enum CMB_STUB_AIF_CTRL ctrl)
{
	...
	if ((state >= CMB_STUB_AIF_MAX)
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] aif_ctrl invalid (%d, %d)\n",
				state, ctrl);
		return -1;
	}
	...
	if (cmb_stub_aif_ctrl_cb) {
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] aif_ctrl_cb null\n");
		ret = -2;
	}
	return ret;
}
...
void mtk_wcn_cmb_stub_func_ctrl(unsigned int type, unsigned int on)
{
	if (cmb_stub_func_ctrl_cb)
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] func_ctrl_cb null\n");
}
...
#endif
{
	...
	if (cmb_stub_thermal_ctrl_cb)
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] thermal_ctrl_cb null\n");
...
void _mtk_wcn_cmb_stub_clock_fail_dump(void)
{
	if (cmb_stub_clock_fail_dump_cb)
		...
		CMB_STUB_LOG_pr_info("[cmb_stub] clock_fail_dump_cb null\n");
}
#endif
...
static int _mt_combo_plt_do_deep_idle(enum COMBO_IF src, int enter)
{
	...
	if (src != COMBO_IF_UART && src != COMBO_IF_MSDC && src != COMBO_IF_BTIF) {
		CMB_STUB_LOG_pr_info("src = %d is error\n", src);
		return ret;
	}
	...
	case COMBO_IF_UART:
		if (enter == 0) {
			...
			if (ret < 0)
				CMB_STUB_LOG_pr_info("%s exit deepidle failed",
						wmt_uart_port_desc);
#endif
#endif
		} else {
			...
			if (ret < 0)
				CMB_STUB_LOG_pr_info("%s enter deepidle failed",
						wmt_uart_port_desc);
#endif
#endif
		}
	...
	case COMBO_IF_BTIF:
		if (cmb_stub_deep_idle_ctrl_cb)
			...
			CMB_STUB_LOG_pr_info("NULL function pointer\n");

		if (ret)
			CMB_STUB_LOG_pr_info("%s deep idle fail(%d)\n",
...
static void mtk_wcn_cmb_sdio_request_eirq(msdc_sdio_irq_handler_t irq_handler, void *data)
{
	...
	if (node) {
		...
		if (ret)
			CMB_STUB_LOG_pr_info("EINT IRQ LINE NOT AVAILABLE!!\n");
		else
			mtk_wcn_cmb_sdio_disable_eirq();/*not ,chip state is power off*/
	} else
		CMB_STUB_LOG_pr_info("[%s] can't find device node\n", __func__);
...
static void mtk_wcn_cmb_sdio_on(int sdio_port_num)
{
	...
	if (mtk_wcn_cmb_sdio_pm_cb) {
		...
		CMB_STUB_LOG_pr_info("mtk_wcn_cmb_sdio_on no sd callback!!\n");
}
...
static void mtk_wcn_cmb_sdio_off(int sdio_port_num)
{
	...
	if (mtk_wcn_cmb_sdio_pm_cb) {
		...
		*/
		...
		CMB_STUB_LOG_pr_info("mtk_wcn_cmb_sdio_off no sd callback!!\n");
...
int board_sdio_ctrl(unsigned int sdio_port_num, unsigned int on)
{
	...
	if (on) {
...
#else
		CMB_STUB_LOG_pr_info("skip sdio off before on\n");
#endif
		/* off -> on */
		...
		if (wifi_irq != 0xffffffff)
			...
			CMB_STUB_LOG_pr_info("wifi_irq is not available\n");
	} else {
			if (wifi_irq != 0xffffffff)
