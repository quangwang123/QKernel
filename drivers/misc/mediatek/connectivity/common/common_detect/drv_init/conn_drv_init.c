...
int do_connectivity_driver_init(int chip_id)
{
	...
	i_ret += tmp_ret;
	if (tmp_ret) {
		WMT_DETECT_pr_info("do common driver not ready, ret:%d\n abort!\n", tmp_ret);
		return i_ret;
	}
	...
	i_ret += tmp_ret;
	if (tmp_ret)
		WMT_DETECT_pr_info("do common driver init failed, ret:%d\n", tmp_ret);
	...
	i_ret += tmp_ret;
	if (tmp_ret)
		WMT_DETECT_pr_info("do fm module init failed, ret:%d\n", tmp_ret);

	tmp_ret = do_wlan_drv_init(chip_id);
	i_ret += tmp_ret;
	if (tmp_ret)
		WMT_DETECT_pr_info("do wlan module init failed, ret:%d\n", tmp_ret);
