...
static long wmt_detect_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	default:
		WMT_DETECT_pr_info("unknown cmd (%d)\n", cmd);
		retval = 0;
		break;
	}
	return retval;
...
int wmt_detect_ext_chip_detect(void)
{
	...
	if (bgfEintStatus == 0) {
		...
		/*Error exists */
		WMT_DETECT_pr_info("error happens when detecting combo chip\n");
		iRet = -3;
	}
	WMT_DETECT_pr_info("--\n");
	/*return 0 */
...
static int wmt_detect_probe(struct platform_device *pdev)
{
	...
	if (-1 == ret)
		WMT_DETECT_pr_info("gpio init fail ret:%d\n", ret);
	return ret;
}
...
static int wmt_detect_driver_init(void)
{
	...
	if (ret)
		WMT_DETECT_pr_info("platform driver register fail ret:%d\n", ret);
#endif

	ret = register_chrdev_region(devID, WMT_DETECT_DEV_NUM, WMT_DETECT_DRVIER_NAME);
	if (ret) {
		WMT_DETECT_pr_info("fail to register chrdev\n");
		goto err0;
	}
